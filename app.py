...
def initialize():
    df = pd.read_csv(
        "https://s3.eu-central-1.amazonaws.com/nyc-crimes-data-app/crimes_app_data.csv"
    )
            ...
            [
                html.Div(
                    [
                        ...
                        html.P(
                            id="total-rides-selection", className="totalRideSelection"
                        ),
                        ...
                        html.Div(
                            [
                                ...
                                html.P(
                                    "Select different months and features of suspectors using \
                            ...
                            [
                                html.Div(
                                    ...
                                    using the dropdown menu",
                                        ...
                                        dcc.Graph(
                                            id="histogram", className="histogramGraph"
                                        ),
                                        html.P(
                                            "",
                                            id="popupAnnotation",
                                            className="popupAnnotation",
                                        ),
                                        dcc.Slider(
                                            id="my-slider",
                                            min=0,
...
def getAge(value):
    if value == None:
        return "18-24"
    val = {"u18": "<18", "u24": "18-24", "u44": "25-44", "u64": "45-64", "a65": "65+"}[
        value
    ]
    return val
...
@app.callback(Output("bar-selector", "value"), [Input("histogram", "selectedData")])
...
def update_total_rides(value, slider_value):
    return "Total # of incidents: {:,d}".format(
        len(totalList[getIndex(value)][slider_value])
    )
...
def update_total_rides_selection(
    value, slider_value, selection, race_value, age_value, sex_value
):
    ...
    for x in selection:
        totalInSelction += len(
            totalList[getIndex(value)][slider_value][
                (totalList[getIndex(value)][slider_value].index.hour == int(x))
                & (
                    totalList[getIndex(value)][slider_value].SUSP_RACE
                    == getRace(race_value)
                )
                & (
                    totalList[getIndex(value)][slider_value].SUSP_AGE_GROUP
                    == getAge(age_value)
                )
                & (
                    totalList[getIndex(value)][slider_value].SUSP_SEX
                    == getSex(sex_value)
                )
            ]
        )
    return "Total incidents in selection: {:,d}".format(totalInSelction)
...
def update_date(value, slider_value, selection):
    ...
    if holder[len(holder) - 1] - holder[0] + 2 == len(holder) + 1 and len(holder) > 2:
        return (
            value,
            " {}".format(getTickLabel(slider_value)),
            " - showing hour(s): ",
            holder[0],
            "-",
            holder[len(holder) - 1],
    ...
    figure = go.Figure(
        data=[
            go.Pie(
                ...
                hole=0.4,
                direction="clockwise",
                textfont=dict(color="rgb(255, 255, 255)", size=12),
                marker=dict(
        ...
        layout=go.Layout(
            ...
            titlefont=dict(size=13, color="rgb(255, 255, 255)"),
            showlegend=False,
            height=285,
...
# just clean
@app.callback(Output("histogram", "selectedData"), [Input("my-dropdown", "value")])
...
@app.callback(Output("popupAnnotation", "children"), [Input("bar-selector", "value")])
...
def update_histogram(value, slider_value, selection, race_value, age_value, sex_value):
    ...
    [xVal, yVal, xSelected, colorVal] = get_selection(
        value, slider_value, selection, race_value, age_value, sex_value
    )
    ...
    return go.Figure(
        data=[
            go.Bar(
                x=xVal,
                y=yVal,
                marker=dict(color=colorVal, line=dict(color="rgb(0, 0, 0)", width=1.2)),
                hoverinfo="x",
            )
        ],
        layout=layout,
...
def get_lat_lon_color(
    selectedData, value, slider_value, race_value, age_value, sex_value
):
        ...
        listStr = (
            "totalList[getIndex(value)][slider_value]\
                   [(totalList[getIndex(value)][slider_value].SUSP_RACE == getRace(race_value)) & \
                    (totalList[getIndex(value)][slider_value].SUSP_AGE_GROUP == getAge(age_value)) & \
                    (totalList[getIndex(value)][slider_value].SUSP_SEX == getSex(sex_value)) & \
                    (totalList[getIndex(value)][slider_value].index.hour >="
...
@app.callback(
    ...
    [State("map-graph", "relayoutData"), State("mapControls", "values")],
)
def update_graph(
    value,
    slider_value,
